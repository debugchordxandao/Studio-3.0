...
const RhythmPanel: React.FC<RhythmPanelProps> = ({ onInsert }) => {
  ...
  const getIcon = (move: RhythmMove) => {
    switch(move) {
      case 'DOWN': return <span className="text-starkids-orange text-3xl font-bold">↓</span>;
      case 'UP': return <span className="text-starkids-blue text-3xl font-bold">↑</span>;
      case 'MUTE': return <span className="text-starkids-red text-2xl font-bold">✕</span>;
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-full">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        >
          <span className="text-3xl text-starkids-red font-bold">✕</span>
        </button>
      ...
      <div className="flex gap-2">
        ...
        <button 
          ...
          className="flex-[2] font-lobster text-lg py-3 bg-starkids-green hover:bg-green-600 disabled:bg-gray-300 disabled:shadow-none text-white rounded-full shadow-[0_3px_0_#2e7d32] active:shadow-none active:translate-y-1 transition-all flex items-center justify-center gap-2"
        >
          Inserir Ritmo 🎵
        </button>
      </div>
