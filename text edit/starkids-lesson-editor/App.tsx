...
import { EditorPage, EditorHandle } from './components/EditorPage';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-8 px-4 md:px-8 print:p-0 print:bg-white">
      <header className="w-full max-w-[1400px] flex flex-col md:flex-row justify-end items-center mb-8 gap-6 z-50 relative">
        <div className="relative" ref={menuRef}>
          <button
            ...
            className={`font-lobster text-xl px-6 py-3 rounded-full text-white transition-all duration-200 transform flex items-center gap-2 bg-starkids-orange shadow-[0_4px_0_#e65100] hover:-translate-y-1 hover:shadow-[0_6px_0_#e65100] active:translate-y-1 active:shadow-none disabled:opacity-70 disabled:cursor-not-allowed`}
          >
      ...
      <footer className="mt-12 text-center font-poppins text-white/90 font-medium drop-shadow-md pb-8">
        <p>Editor de Aulas Interativo • Starkids Studio</p>
        <p className="text-sm opacity-75 mt-1">Design by XANDÃO</p>
      </footer>
    </div>
  );
};
