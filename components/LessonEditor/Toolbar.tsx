    ...
    return (
        <div className="relative z-10 bg-white p-4 rounded-[30px] shadow-xl border-4 border-white flex flex-wrap gap-4 items-center justify-center w-full max-w-4xl mx-auto mb-8 transition-all">
            ...
            <div className="flex items-center gap-2 bg-sky-50 px-3 py-2 rounded-full border border-sky-100">
                <button onClick={() => onExecCommand('bold')} className="p-2 hover:bg-white hover:text-sky-500 rounded-full transition-colors text-gray-600" title="Negrito">
                    <Bold size={20} />
                </button>
                <button onClick={() => onExecCommand('italic')} className="p-2 hover:bg-white hover:text-sky-500 rounded-full transition-colors text-gray-600" title="Itálico">
                    <Italic size={20} />
                </button>
                <button onClick={() => onExecCommand('underline')} className="p-2 hover:bg-white hover:text-sky-500 rounded-full transition-colors text-gray-600" title="Sublinhado">
                    <Underline size={20} />
                </button>
            </div>
            ...
            <div className="flex items-center gap-2 bg-sky-50 px-3 py-2 rounded-full border border-sky-100">
                ...
                {FONT_SIZES.map((size) => (
                    <button
                        ...
                        className={`font-poppins font-semibold text-gray-600 hover:text-purple-500 transition-colors px-2 ${size.value === '3' ? 'text-sm' : size.value === '5' ? 'text-lg' : 'text-2xl'}`}
